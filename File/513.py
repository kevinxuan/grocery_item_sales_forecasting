...
def prepare_dataset(t2017, is_train=True):
    ...
    # number of weeks of mean for every Monday, Tuesday, ... (past)
    for i in range(7):
        ...
        datex = get_nearwd(t2017+timedelta(i),t2017)
        ahead = (t2017-datex).days
        if ahead!=0:
            X['ahead0_{}'.format(i)] = get_timespan(df_2017, datex+timedelta(ahead), ahead, ahead).mean(axis=1).values
            X['ahead7_{}'.format(i)] = get_timespan(df_2017, datex+timedelta(ahead), ahead+7, ahead+7).mean(axis=1).values
        # sales number for 1 day and 8 days before
        X["day_1_2017_{}1".format(i)]= get_timespan(df_2017, datex, 1, 1).values.ravel()
   #     X["day_1_2017_{}2".format(i)]= get_timespan(df_2017, date-timedelta(7), 1, 1).values.ravel()
        for m in [3,7,30,140]:
            X["mean_{}_2017_{}1".format(m,i)]= get_timespan(df_2017, datex,m, m).\
                mean(axis=1).values
            X["mean_{}_2017_{}2".format(m,i)]= get_timespan(df_2017, datex-timedelta(7),m, m).\
                mean(axis=1).values
    if is_train:
        y = df_2017[
...
del item_nbr_u
submission[['id','unit_sales']].to_csv('lgb_6wks.csv', float_format='%.4f', index=None)
'''